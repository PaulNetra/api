...
x = ''
@app.route('/api/etl') # App route declaration for ETL-API.
def run_etl_process():
    # ***
...
#        key    = flsk.request.args.get('key')
    ...
    args = [fin, fout]
    args = ['/media/Joe-Jalal/API/test_update3/tst_src3/', 
...
#    apilog.LogStart(flog, 'etl', folder=True, PID=False)
    apilog.LogStart(flog, 'etl', infile=args[0], outfile=args[1], folder=True, PID=False)
...
#    return flsk.jsonify(x)
    return apilog.API_Response(appkey, key, flog, apilog.STATUS_OK,"","etl started", {})
...
def run_dpc_process():
    try:
        ...
        print(type(threshold), type(scoop_size), type(file_name)) # Print statements for debugging on my end.
    except Exception as e:
        print('Exception', e)
        return apilog.API_Response(appkey="",key="", status=apilog.URL_ERROR) # Flask returns json of map_data; either through curl or web browser.
    ...
    if threshold==0 or scoop_size<16:
        ...
        global data_
        try:
            pid = dpcon.main_process_pid(file_name, threshold, scoop_size, appkey, key, flog) # Calls main_process in DpcController.py (DeepCrush's controller module).
            print('Main process initiated via API')
            return apilog.API_Response(appkey, key, flog, apilog.STATUS_OK,"","deepcrush started", {})
...
# SCRIPT NEVER BE HERE
    ...
    return apilog.API_Response(appkey, key, flog, apilog.EXIT_OK,"","deepcrush finished", {})


@app.route('/api/deepcrush-process') # App route declaration for DeepCrush-API.
...
def deepcrush_response(appkey, key, task):
    url= "http://127.0.0.1:5000/api/deepcrush-process?appkey=%s&key=%s&task=%s"%(appkey, key,task)
    r = requests.get(url)
    return flsk.Response(
        r.text,
        status=r.status_code,
    ...
    #return API_process(appkey, key, task)

##################################################
# End of DeepCrush Section
##################################################
...
if __name__ == '__main__':
    with app.test_request_context(): # For debugging; used to see correct api_endpoint for DeepCrush.
        print(flsk.url_for('run_var_process', threshold=40, scoop_size=16, file_name='1077_lung_set.csv'))
